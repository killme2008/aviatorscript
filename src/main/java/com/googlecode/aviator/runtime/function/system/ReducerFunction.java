...
/**
 * Internal reducer function for 'for' and 'loop' structure.
 *
 * @since 5.0.0
...
public class ReducerFunction extends AbstractFunction {
  ...
  public AviatorObject call(final Map<String, Object> env, final AviatorObject arg1,
    ...
    AviatorObject result = new ReducerResult(ReducerState.Cont, AviatorNil.NIL);

    Class<?> clazz = coll.getClass();

    if (Iterable.class.isAssignableFrom(clazz)) {
      for (Object obj : (Iterable<?>) coll) {
        result = fun.call(env, AviatorRuntimeJavaType.valueOf(obj));
        if (result.isNull(env)) {
          continue;
        }
