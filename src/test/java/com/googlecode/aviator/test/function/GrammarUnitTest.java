...
public class GrammarUnitTest {
	...
	public void testType() {
		...
		assertEquals("hello \" world",
				AviatorEvaluator.execute("'hello \" world'"));
		assertEquals("hello 'world'",
				AviatorEvaluator.execute("\"hello 'world'\""));
		assertEquals("hello 'world' 'dennis'",
				AviatorEvaluator.execute("\"hello 'world' 'dennis'\""));

		assertTrue((Boolean) AviatorEvaluator.execute("true"));
		assertFalse((Boolean) AviatorEvaluator.execute("false"));

		assertEquals("/\\w+\\d?\\..*/",
				AviatorEvaluator.execute("/\\w+\\d?\\..*/"));
	...
	public void testTypeConversation() {
		...
		assertTrue(AviatorEvaluator
				.execute("3.19+3.1/2.9-1.0/(6.0002*7.7+8.9)") instanceof Double);
		...
		assertEquals("hello world",
				AviatorEvaluator.execute("'hello '+ 'world'"));
		assertEquals("hello aviator",
				AviatorEvaluator.execute("'hello '+tmap.key", env));
		assertEquals("true aviator",
				AviatorEvaluator.execute("true+' '+tmap.key", env));
		assertEquals("100aviator",
				AviatorEvaluator.execute("foo.a+tmap.key", env));
		assertEquals("\\d+hello", AviatorEvaluator.execute("/\\d+/+'hello'"));
		assertEquals("3.2aviator",
				AviatorEvaluator.execute("3.2+tmap.key", env));
		assertEquals("false is false",
				AviatorEvaluator.execute("bool+' is false'", env));
	...
	public void testAddOperandsLimit() {
		...
		assertEquals("hello aviator",
				AviatorEvaluator.execute("'hello '+s", env));
	...
	public void testSubOperandsLimit() {
		...
		assertEquals(1003.3, (Double) AviatorEvaluator.execute("a-d", env),
				0.001);
	...
	public void testMultOperandsLimit() {
		...
		assertEquals(10.89, (Double) AviatorEvaluator.execute("d*d", env),
				0.001);
		assertEquals(-3300, (Double) AviatorEvaluator.execute("a*d", env),
				0.001);
		doArithOpIllegalOperands("*");
	}
	...
	public void testDivOperandsLimit() {
		...
		assertEquals(-303.030, (Double) AviatorEvaluator.execute("a/d", env),
				0.001);
		doArithOpIllegalOperands("/");
	}
	...
	public void testModOperandsLimit() {
		...
		assertEquals(2.86, (Double) AviatorEvaluator.execute("6%3.14"), 0.001);
		assertEquals(-0.29999, (Double) AviatorEvaluator.execute("d%3", env),
				0.001);
		assertEquals(0.0, (Double) AviatorEvaluator.execute("d%d", env), 0.001);
		assertEquals(1000 % -3.3,
				(Double) AviatorEvaluator.execute("a%d", env), 0.001);
		doArithOpIllegalOperands("%");
	}
	...
	public void testMatch() {
		...
		assertTrue((Boolean) AviatorEvaluator
				.execute("'10'=~/^[0-9]*[1-9][0-9]*$/"));
		assertTrue((Boolean) AviatorEvaluator
				.execute("'1'=~/^[0-9]*[1-9][0-9]*$/"));
		assertFalse((Boolean) AviatorEvaluator
				.execute("'0'=~/^[0-9]*[1-9][0-9]*$/"));
		assertFalse((Boolean) AviatorEvaluator
				.execute("'-3'=~/^[0-9]*[1-9][0-9]*$/"));
		assertFalse((Boolean) AviatorEvaluator
				.execute("'aviator'=~/^[0-9]*[1-9][0-9]*$/"));
		...
		assertTrue((Boolean) AviatorEvaluator
				.execute("'455729032'=~/^[1-9]*[1-9][0-9]*$/"));
		assertFalse((Boolean) AviatorEvaluator
				.execute("'45d729032'=~/^[1-9]*[1-9][0-9]*$/"));
		assertTrue((Boolean) AviatorEvaluator
				.execute("'<html>hello</html>'=~/<(.*)>.*<\\/\\1>|<(.*) \\/>/"));
		assertTrue((Boolean) AviatorEvaluator
	...
	public void testTernaryOperator() {
		...
		assertEquals(f, (Float) AviatorEvaluator.execute("false?i:f", env),
				0.001);
		assertEquals(i, AviatorEvaluator.execute("true?i:f", env));
		assertEquals("killme2008", AviatorEvaluator.execute(
				"email=~/([\\w0-9]+)@\\w+\\.\\w+/ ? $1:'unknow'", env));

		assertEquals(f, (Float) AviatorEvaluator.execute("ch!='a'?i:f", env),
				0.001);
		...
		// 使用括号
		assertEquals(email, AviatorEvaluator.execute(
				"!t? (i>0? f:ch) :( f>3?email:ch)", env));
		// 测试错误情况
		try {
	...
	public void testIndex() {
		...
		for (int i = 0; i < a.length; i++) {
			...
			assertEquals(a[i] + i,
					AviatorEvaluator.execute("a[" + i + "]+" + i, env));
			assertEquals(a[i] + i, AviatorEvaluator.execute("a[a[a[" + i
					+ "]-10+10]]+" + i, env));
		}
		...
		assertEquals("hello world",
				AviatorEvaluator.execute("list[0]+' '+list[1]", env));
	...
	public void testFunctionCall() {
		...
		assertEquals(
				10,
				AviatorEvaluator
						.execute(" string.length('hello') + string.length('hello') "));
		assertEquals(3.0, (Double) AviatorEvaluator.execute(
				" string.length('hello')>5? math.abs(d):math.log10(a) ", env),
				0.001);
		...
		assertEquals(
				3.3,
				(Double) AviatorEvaluator
						.execute(
								"string.contains(p1,'A-Z')? d<0? math.abs(d): math.sqrt(a) : string.length(p2)  ",
								env), 0.001);
	}
